...
void RtuStream::writeBegin(quint8 slave)
{
    m_data.clear();
    m_data.append(slave);
}

void RtuStream::writeByte(quint8 value)
{
    m_data.append(value);
}

void RtuStream::writeWord(quint16 value)
{
    m_data.append(quint8(value >> 8));
    m_data.append(quint8(value));
}

void RtuStream::writeEnd()
{
    const quint16 value = checksum(m_data);

    m_data.append(quint8(value));
    m_data.append(quint8(value >> 8));
}

QByteArray RtuStream::data() const
{
    return m_data;
}
...
quint16 RtuStream::checksum(const QByteArray &data)
{
    ...
    foreach (quint8 value,data)
        result = (result >> 8) ^ table[quint8(result ^ value)];
